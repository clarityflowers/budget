...
pub fn render(self: *@This(), input_key: ?ncurses.Key) !bool {
    var input = input_key;
    ...
    // window.wrap = true;
    const number_to_display = (@intCast(usize, window.getMaxPosition().line) / 4) - 1;
    ...
    for (transactions[start..]) |transaction, i| {
        const row = i * 4 + 1;
        if (row + 4 > window.getMaxPosition().line - 3) break;
    ...
    const max_y = window.getMaxPosition().line;
    window.move(.{ .line = max_y - 3 });
    window.fillLine('═') catch {};
    window.move(.{ .line = max_y - 2 });
    ...
    if (self.attempting_interrupt) {
        writer.print("Press ^C again to quit.", .{}) catch {};
    } else {
        switch (self.state.field) {
            .payee => |*payee_editor| {
                if (try payee_editor.render(input, window)) {
                    input = null;
                }
            },
            else => {},
